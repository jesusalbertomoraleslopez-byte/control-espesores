...
# 1. CONFIGURACIÓN ÚNICA DE LA PÁGINA (Previene errores de duplicado)
st.set_page_config(page_title="Suite de Riesgo y Control de Espesores", layout="wide")

# 2. CONFIGURACIÓN GRÁFICA VECTORIAL PARA NUBE (ReportLab y Matplotlib)
...
import matplotlib
matplotlib.use('Agg') # Renderizado seguro sin servidor gráfico X11
import matplotlib.pyplot as plt

# 3. MATRIZ TÉCNICA PLANTA (Límites rígidos de diseño)
ESTANDAR = {
...
# Banner corporativo desde la raíz del proyecto
st.image("BANNER CONTROL DE ESPESORES APP.png", use_container_width=True)
def calcular_riesgo_grupos(df_entrada, tol_proveedor):
    """Calcula el área bajo la curva normal para hallar el porcentaje de riesgo de rechazo global por grupo."""
    ...
    for (m, c), sub in df_entrada.groupby(['Material', 'Calibre']):
        ...
        # Área fuera de los límites de tolerancia estándar (Nominal ± TOLERANCIA_INTERNA)
...
def colorear_matriz_resumen(v):
    """Aplica formato semafórico condicional basado en las alertas de riesgo."""
...
def generar_excel_plantilla():
    """Construye el archivo binario de descarga para la plantilla estandarizada en RAM."""
    datos = {
...
def crear_pdf_formal(df_final, resumen, tol_p):
    ...
    story.append(Paragraph("<b>Documento:</b> Reporte Técnico de Ingeniería de Calidad", m_st))
    ...
    # Tabla 1: Criterios Teóricos de Diseño
    story.append(Paragraph("1. Especificaciones Técnicas y Criterios de Aceptación Interna", h2_st))
    t_req_d = [[Paragraph("Material / Calibre", h_style), Paragraph("Espesor Nominal", h_style), Paragraph("Límite Mínimo", h_style), Paragraph("Límite Máximo", h_style)]]
    for r in resumen:
    ...
    t_1.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#708090')), ('ALIGN', (0,0), (-1,-1), 'CENTER'), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#A0A0A0'))]))
    story.append(t_1)
    
    # Tabla 2: Diagnóstico Probabilístico Consolidado
    ...
    t_2.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#2b579a')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#A0A0A0'))] + est_riesgo))
    story.append(t_2)
    
    # Tabla 3: Muestreo Físico Unidad por Unidad
    story.append(Paragraph("3. Desglose Analítico por Unidad (Muestreo Físico)", h2_st))
    t_rollos_d = [[Paragraph("Rollo", h_style), Paragraph("Material", h_style), Paragraph("Calibre", h_style), Paragraph("Espesor (in)", h_style), Paragraph("Desviación", h_style), Paragraph("Estatus Planta", h_style)]]
    est_rollos = []
    ...
    t_3.setStyle(TableStyle([('BACKGROUND', (0,0), (-1,0), colors.HexColor('#2b579a')), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#D3D3D3'))] + est_rollos))
    story.append(t_3)
    ...
    try:
        ...
        story.append(Paragraph(f"<i>No se pudo renderizar la gráfica: {str(e)}</i>", c_style))
    ...
    buffer.seek(0)
    return buffer
# CONTROLES LATERALES O SUPERIORES DE LA SUITE
...
if archivo_cargado is not None:
    try:
        ...
        # Bucle de Limpieza y Homologación de Unidades
        ...
        if not df_datos_cargados.empty:
            # ======================================================================
            # NUEVO ALGORITMO: EVALUACIÓN DE RIESGO ESTADÍSTICO INDIVIDUAL (GAUSS)
            # ======================================================================
            est_l = []
            sigma_individual = tol_proveedor / 3.0  # Desviación típica basada en variabilidad de proveedor
            
            for _, fila in df_datos_cargados.iterrows():
                med_individual = fila['Espesor Real (in)']
                nom_individual = fila['Nominal Estándar']
                
                # Cálculo de integrales normales en colas de la distribución (fuera de ±0.008")
                ...
                # Clasificación matemática pura en base a la probabilidad de fallo
            ...
            # --- RENDERIZADO DE TABLA 1: MUESTREO INDIVIDUAL ROLLO POR ROLLO ---
            ...
            styler_individual = df_datos_cargados.style.format(formatos).map(colorear_matriz_resumen, subset=['Estatus Planta'])
            st.dataframe(styler_individual, use_container_width=True)
            
            # --- RENDERIZADO DE TABLA 2: MATRIZ DE RIESGO AGRUPADA ---
            ...
            styler_resumen = df_resumen.style.map(colorear_matriz_resumen, subset=['Dictamen'])
            st.dataframe(styler_resumen, use_container_width=True)
            
            # --- RENDERIZADO DE GRÁFICA WEB DINÁMICA (PLOTLY) ---
            ...
            # --- ACCIÓN DE EXPORTACIÓN FINAL A PDF CORPORATIVO ---
